...
def check_win(grid, player):
    ...
    for row in range(len(grid)):
        for col in range(len(grid[0])):
            ...
            if row - 3 > len(grid) - 4 and col + 3 < len(grid[row]):
                if grid[row][col] == player\
                   and grid[row-1][col+1] == player\
                   and grid[row-2][col+2] == player\
                   and grid[row-3][col+3] == player:
    ...
    for row in range(len(grid)):
        for col in range(len(grid[0])):
            ...
            if row + 3 <= len(grid) - 1 and (col - 3) >= 0:
                if grid[row][col] == player\
                   and grid[row+1][col-1] == player\
                   and grid[row+2][col-2] == player\
                   and grid[row+3][col-3] == player:
...
def play(x_pos, y_pos):
    ...
    if check_win(grid, 1):
        print("player 1 won")
        window.bye()
        w.open('https://www.youtube.com/watch?v=WPzMxiGd0io')
...
def main():
    ...
    while True:
        ...
        if turn == 1 and grid[selected_row][selected_col] == 0:
            grid[4 - selected_row][selected_col] = 1
        elif turn == 2 and grid[selected_row][selected_col] == 0:
            grid[4 - selected_row][selected_col] = 2
            print(grid)
